...
router.post('/', async (req, res) => {
  try {
    ...
    const {
      ...
      latitude, longitude, default_radius_meters, geofence_strict, geofence_require_photo,
    } = req.body;
    ...
    const { rows } = await query(
      ...
      [req.user.organization_id, name.trim(), trade_name || null, cnpj || null,
        ...
        latitude ?? null, longitude ?? null, default_radius_meters ?? 200,
        !!geofence_strict, !!geofence_require_photo]
    );
